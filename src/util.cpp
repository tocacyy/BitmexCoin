...
int nAnonymizeMagnetAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Magnet
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Magnet
    // Mac: ~/Library/Application Support/Magnet
    // Unix: ~/.bitmexcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Magnet";
#else
    fs::path pathRet;
    ...
    return pathRet / "Magnet";
#else
    // Unix
    return pathRet / ".bitmexcoin";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "bitmexcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "bitmexcoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong BitmexCoin will not work properly.");
