...
const char * const BITCOIN_CONF_FILENAME = "BitmexCoin.conf";
const char * const BITCOIN_PID_FILENAME = "BitmexCoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "BitmexCoin" is a composite category enabling all BitmexCoin-related debug output
            if(ptrCategory->count(string("BitmexCoin"))) {
...
#else
    const char* pszModule = "BitmexCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitmexCoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitmexCoinCore
    // Mac: ~/Library/Application Support/BitmexCoinCore
    // Unix: ~/.BitmexCoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitmexCoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitmexCoinCore";
#else
    // Unix
    return pathRet / ".BitmexCoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty BitmexCoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override BitmexCoin.conf
